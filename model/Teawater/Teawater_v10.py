...
class En_blocks2(nn.Module):
    def __init__(self, in_channel, out_channel,decay=1):
        super(En_blocks2, self).__init__()
        self.conv1 = nn.Sequential(
        ...
        self.conv2 = nn.Sequential(
            nn.Conv2d(out_channel//2, out_channel//2, 3, padding=1),
            nn.BatchNorm2d(out_channel//2),
            nn.ReLU(inplace=True)
        )
    ...
    def forward(self, x):
        conv1 = self.conv1(x)
        conv2 = self.conv2(conv1)
        att=self.sapceatt(conv1,conv2)
        out=torch.cat([conv2,att],dim=1)
        return out
...
class Teawater_v9(nn.Module):
    def __init__(self, n_class=1,decay=2):
        super(Teawater_v9, self).__init__()
        self.pool = nn.MaxPool2d(2)

        self.down_conv1 = En_blocks2(3, 64,decay)
        self.down_conv2 = En_blocks2(64, 128,decay)
        self.down_conv3 = En_blocks2(128, 256,decay)
        self.down_conv4 = En_blocks2(256, 512,decay)
        self.down_conv5 = En_blocks2(512, 1024,decay)
        #self.center = Center()
