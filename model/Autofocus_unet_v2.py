...
class Autofocus_conv(nn.Module):
    def __init__(self,input_channel,output_channel,num_branches=3):
        ...
        self.conv11=nn.Conv2d(input_channel,output_channel//2,kernel_size=3,dilation=2,padding=2)
        self.conv12=nn.Conv2d(input_channel,output_channel//2,kernel_size=3,dilation=6,padding=6)
        self.conv13=nn.Conv2d(input_channel,output_channel//2,kernel_size=3,dilation=10,padding=10)
        #self.conv14=nn.Conv2d(input_channel,output_channel//2,kernel_size=3,dilation=14,padding=14)
        self.convatt11=nn.Conv2d(input_channel,input_channel//2,kernel_size=3,dilation=2,padding=2)
        self.convatt12=nn.Conv2d(input_channel//2,out_channels=num_branches,kernel_size=1)
       
        self.conv21=nn.Conv2d(output_channel//2,output_channel,kernel_size=3,dilation=2,padding=2)
        self.conv22=nn.Conv2d(output_channel//2,output_channel,kernel_size=3,dilation=6,padding=6)
        self.conv23=nn.Conv2d(output_channel//2,output_channel,kernel_size=3,dilation=10,padding=10)
        #self.conv24=nn.Conv2d(output_channel//2,output_channel,kernel_size=3,dilation=14,padding=14)

        self.convatt21=nn.Conv2d(output_channel//2,output_channel//2,kernel_size=3,dilation=2,padding=2)
        self.convatt22=nn.Conv2d(output_channel//2,num_branches,kernel_size=1)
    ...
    def forward(self,x):
        ...
        x23=self.Bn2(x23)
        #x24=self.Bn(x24)
        x21=x21*att2[:,0:1,:,:]+x22*att2[:,1:2,:,:]+x23*att2[:,2:3,:,:]
        x21=self.relu(x21)
        x = self.relu(x21)
        return x
