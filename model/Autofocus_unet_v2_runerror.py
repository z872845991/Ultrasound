...
class Autofocus_conv(nn.Module):
    def __init__(self,input_channel,output_channel,num_branches=4):
        ...
        self.convatt11=nn.Conv2d(input_channel,int(input_channel/2),kernel_size=3,dilation=2)
        ...
        self.convatt21=nn.Conv2d(output_channel,int(output_channel/2),kernel_size=3,dilation=2)
    ...
    def forward(self,x):
        ...
        x13=self.Bn(x13)
        x14=self.Bn(x14)
        x11=x11*att[:,0:1,:,:]+x12*att[:,1:2,:,:]+x13*att[:,2:3,:,:]+x14*att[:,3:4,:,:]
        x=self.relu(x11)
        ...
        x23=self.Bn(x23)
        x24=self.Bn(x24)
        x21=x21*att2[:,0:1,:,:]+x22*att2[:,1:2,:,:]+x23*att2[:,2:3,:,:]+x24*att2[:,3:4,:,:]
        x21=self.relu(x21)
        x = self.relu(21)
        return x
